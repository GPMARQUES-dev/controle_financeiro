    ...
    try {
        ...
        async function addTransaction() {
            ...
            try {
                ...
                // Recarrega os dados do servidor para mostrar a nova transação
        ...
        async function deleteTransaction(idToDelete) {
            if (confirm('Tem certeza que deseja excluir esta transação?')) {
                try {
                    const response = await fetch(`http://127.0.0.1:5000/api/transacoes/${idToDelete}`, {
                        method: 'DELETE'
                    });
                    ...
                    // Após excluir, recarrega a lista do servidor para mostrar a mudança
                    await loadTransactions();
                    saveAndRerender();
        ...
        async function saveEditedTransaction(idToSave) {
            const row = document.querySelector(`tr[data-transaction-id='${idToSave}']`);
            if (!row) return;

            // Pega os novos dados dos campos de input na tabela
            const newType = row.querySelector('.edit-type').value;
            const newDate = row.querySelector('.edit-date').value;
            const newValue = parseFloat(row.querySelector('.edit-value').value);
            const newCategory = row.querySelector('.edit-category').value;
            const newLocation = row.querySelector('.edit-location').value;

            if (isNaN(newValue) || newValue <= 0 || !newDate) {
                alert('Os valores para data e valor não podem estar vazios.');
                return;
            }

            // Monta o objeto de dados com os nomes em inglês que a API espera
            const updatedData = {
                type: newType,
                date: newDate,
                value: newValue,
            ...
            try {
                const response = await fetch(`http://127.0.0.1:5000/api/transacoes/${idToSave}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
